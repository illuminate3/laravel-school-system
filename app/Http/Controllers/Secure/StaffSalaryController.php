...
class StaffSalaryController extends SecureController
{
    ...
    public function __construct(StaffSalaryRepository $staffSalaryRepository)
    {
        ...
        $this->middleware('authorized:staff_salary.show', ['only' => ['index', 'data']]);
        $this->middleware('authorized:staff_salary.create', ['only' => ['create', 'store']]);
        $this->middleware('authorized:staff_salary.edit', ['only' => ['update', 'edit']]);
        $this->middleware('authorized:staff_salary.delete', ['only' => ['delete', 'destroy']]);
    ...
    public function data(User $teacher)
    {
        ...
        return Datatables::of($staff_salaries)
            ->add_column('actions', '@if(!Sentinel::getUser()->inRole(\'admin\') || Sentinel::getUser()->inRole(\'super_admin\') || (Sentinel::getUser()->inRole(\'admin\') && Settings::get(\'multi_school\') == \'no\') || (Sentinel::getUser()->inRole(\'admin\') && in_array(\'staff_salary.edit\', Sentinel::getUser()->permissions)))
                                    ...
                                    @if(!Sentinel::getUser()->inRole(\'admin\') || Sentinel::getUser()->inRole(\'super_admin\') || (Sentinel::getUser()->inRole(\'admin\') && Settings::get(\'multi_school\') == \'no\') || (Sentinel::getUser()->inRole(\'admin\') && in_array(\'staff_salary.delete\', Sentinel::getUser()->permissions)))

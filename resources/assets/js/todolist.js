            ...
            $.ajax({
                ...
                success: function (id) {
                    ...
                    if (task_from_user != $("#user_id").val()) {
                        $(".list_of_items").append("<div class='todolist_list showactions list1' id='" + id + "'>  " + "<div class='col-md-8 col-sm-8 col-xs-8 nopadmar custom_textbox1'> <div class='todoitemcheck'><input type='checkbox' class='striked ' autocomplete='off' /</div>" + "<div class='todotext todoitemjs'>" + $("#task_description").val() + " </div> <span class='label label-default'>" + $("#task_deadline").val() + "</span> <span class='label label-info'>" + $("#user_id option:selected").text() + "</span></div>" + twoButtons);
                    }

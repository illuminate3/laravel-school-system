...
class SalaryController extends SecureController
{
    ...
    public function print_salary(Salary $salary)
    {
        ...
        $pdf->loadView('report.salary', compact('data'));
        return $pdf->stream();
    }
    ...
    public function data()
    {
	    $salaries = $this->salaryRepository
		    ->getAllForSchoolYearSchool( Session::get( 'current_school'), Session::get( 'current_school_year' ) )
		    ->with('user')
            ->get()
            ->map(function ($salary) {
                return [
        ...
        return Datatables::of($salaries)
            ->add_column('actions', '@if(!Sentinel::getUser()->inRole(\'admin\') || Sentinel::getUser()->inRole(\'super_admin\') || (Sentinel::getUser()->inRole(\'admin\') && Settings::get(\'multi_school\') == \'no\') || (Sentinel::getUser()->inRole(\'admin\') && in_array(\'salary.edit\', Sentinel::getUser()->permissions)))
    ...
    public function list_of_users() {
	    ...
	    if ( $one_school && $this->user->inRole( 'accountant' ) ) {
		    $teachers = $this->teacherSchoolRepository->getAllForSchool( Session::get( 'current_school' ) )
		                                              ->map( function ( $user ) {
			                                              return [
				                                              'id'   => $user->id,
				                                              'name' => $user->full_name,

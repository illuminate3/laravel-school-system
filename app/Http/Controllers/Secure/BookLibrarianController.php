...
class BookLibrarianController extends SecureController
{
    ...
    public function issueReturnBook(User $user)
    {
        ...
        if ($user == null) {
            ...
            $bookuserb = $this->getBookRepository->getAllForUser($user->id)
                ->groupBy('book_id')
                ->with('book')
                ->select('get_books.*', DB::raw('sum(get_books) as book_get_sum'))
                ->get()
    ...
    {
        if(is_int($return_books)) {
            $return_book = ReturnBook::create(['book_id' => $getBook->book_id,
            ...
            if (Settings::get('late_return_book_make_invoice') == 'true' && isset($getBook->book->borrowingPeriod)) {
                ...
                if ($difference > $getBook->book->borrowingPeriod->value) {
                    Invoice::create(['title' => trans('booklibrarian.book_late_return'),
    ...
    {
        if(is_int($get_books)) {
            $bookUser = new GetBook();
            $bookUser->user_id = $user->id;
            $bookUser->book_id = $book->id;
            $bookUser->get_date = date('Y-m-d');
            $bookUser->get_books_count = $get_books;
            $bookUser->school_year_id = session('current_school_year');
            $bookUser->save();
        }
        $userb = $user;
